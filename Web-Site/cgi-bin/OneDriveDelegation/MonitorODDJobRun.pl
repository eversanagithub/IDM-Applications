...
#my $associateName = "jenika.macias@eversana.com";
#my $requesterName = "dave.jaynes@eversana.com";
#my $Incident = "";

my $NiceAssociateName = ParsingNiceNames($associateName);
my $NiceRequesterName = ParsingNiceNames($requesterName);
$NiceAssociateName =~ s/~/ /g;
$NiceRequesterName =~ s/~/ /g;
$NiceAssociateName = $NiceAssociateName . "'s";
...
print "<tr>\n";
print "<th width='25%' align='center'>\n";
print "<p class='IDMTermedReportTitle'>Assigning access to $NiceRequesterName for $NiceAssociateName One-Drive account</p>\n";
print "</th>\n";
print "</table>\n";
print "<table width='100%' align='center'>\n";
print "<tr>\n";
...
sub ParsingNiceNames
{
	my ($name) = @_;
	my @FirstLastNameArray = split('\.',$name);
	my $FirstName = $FirstLastNameArray[0];
	my $LastName = $FirstLastNameArray[1];
	$FirstName =~ s/(\w+)/\u$1/g;
	$LastName =~ s/(\w+)/\u$1/g;
	my $TempName1 = $FirstName . "~" . $LastName;
	my @TempName2  = split('\@',$TempName1);
	my $ReturnName = $TempName2[0];
	return $ReturnName;
}
