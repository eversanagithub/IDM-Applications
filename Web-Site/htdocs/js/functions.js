     ...
     Date Written: May 8th, 2023
       Written By: Dave Jaynes
          Purpose: This file holds all the functions for the IDM Website.
                   It is broken down into eleven sections, each one cooresponding to a different functionality of the website.
                   These eleven sections are:
...
const InitialJobCodeDropDownListPHPFile = "http://idmgmtapp01/php/InitialJobCodeDropDownList.php";
const JobCodeDropDownListPHPFile = "http://idmgmtapp01/php/JobCodeDropDownList.php";
const DepartmentCodeDropDownListURL = "http://idmgmtapp01/php/DepartmentCodeDropDownList.php";
...
const PullApplicationPath = "http://idmgmtapp01/php/HousekeepingScripts/PullApplicationPath.php";
const DisplayNoAccessToApplication = "http://idmgmtapp01/php/BuildWebPageScripts/DisplayNoAccessToApplication.php";
const InitiateTerminationDropDownPHPFile = "http://idmgmtapp01/php/OpenApplications/GetTerminationDropDown.php";
const UpdateTerminationDropDownPHPFile = "http://idmgmtapp01/php/OpenApplications/UpdateTerminationDropDown.php";
const GetFormerAssociateInfoPHPFile = "http://idmgmtapp01/php/OpenApplications/GetFormerAssociateInfo.php";
/*
===================================================================================================
...
const CallRetrieveApplicationPath = (application,applicationURL,returnAppPath) => {
	if(user == '' || user == null) { user = 'Empty'; }
	if(EncryptedKey == '' || EncryptedKey == null) { EncryptedKey = 'Empty'; }
	let getInfoParams = 'user=' + user + '&EncryptedKey=' + EncryptedKey;
	const requestAppPath = new XMLHttpRequest();
	requestAppPath.addEventListener('readystatechange', () => {
		if(requestAppPath.readyState === 4 && requestAppPath.status === 200) 
		{
			returnAppPath(requestAppPath.responseText);
		}
	});
	requestVerificationCode.open("POST", applicationURL, true);
	requestVerificationCode.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	requestVerificationCode.send(getInfoParams);
}

const CallUserBasedApplication = (user,EncryptedKey,applicationName,applicationURL) => {	
	let params = 'user=' + user + '&EncryptedKey=' + EncryptedKey + '&applicationName=' + applicationName;
	sendGoodRequest = new XMLHttpRequest();
...
const CallAPNoAccessToApplication = (application,getAppPath,getReturnValue,applicationURL) => {	
	let params = 'application=' + application + '&applicationPath=' + applicationPath + '&errorCode=' + getReturnValue;
	sendGoodRequest = new XMLHttpRequest();
	sendGoodRequest.open("POST", applicationURL, true);
	sendGoodRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	sendGoodRequest.send(params);
...
const DisplayComplianceNotificationIntro = () => {
	// Called by the SearchBoxes.html file.
	top.topmainpanel.location='http://idmgmtapp01/webpages/ComplianceNotificationHeader.htm';
}
		...
		CheckUsersAuthentication(user,EncryptedKey,CheckUsersAuthenticationURL,function(GetReturnValue)
		{
			...
			switch(GetReturnValue)
			{
				case "0":
					...
					break;
				case "63":
					CallUserBasedApplication(user,EncryptedKey,application,applicationURL);
					break;
				default:
					AdminPortalErrorScreen(user,EncryptedKey,GetReturnValue,AdminPortalErrorScreenURL);
					break;
...
const CreateAPApplicationHTMLResponse = (application) => {
	...
	GetAPApplicationURL(application,GetAPApplicationURLValue,function(applicationURL)
	{
		...
		{
			getReturnValue = getReturnValue.trim();
			if(getReturnValue == appAuthorized)
			{
				CallUserBasedApplication(user,EncryptedKey,application,applicationURL);
			}
			else
			{
				CallRetrieveApplicationPath(application,PullApplicationPath,function(getAppPath)
				{
					getAppPath = getAppPath.trim();
					CallAPNoAccessToApplication(application,getAppPath,getReturnValue,DisplayNoAccessToApplication);
				});
			}
		});
	});
...
function StatusOfODDProcess()
{
	...
	request.addEventListener('readystatechange', () => {
		if(request.readyState === 4 && request.status === 200) {
			// Populate the str variable with the extracted JSON data.
			// { "oddstats" : [{ "TermedUser":"alexandra.moore.com", "RequestingUser":"dave.jaynes.com", "DateTimeProcessed":"2023-10-16 13:46:43.000", "OverallStatus":"Waiting", "CurrentModuleProcessing":"Waiting" } ]}
			const str = request.responseText;
			const obj = JSON.parse(str);
			...
			{
				case "Waiting":
					imgLocation = "http://idmgmtapp01/images/ODDWaitingForJobToBegin.jpg";
					break;
				case "Initializing":
					imgLocation = "http://idmgmtapp01/images/ODDInitializingJobVariables.jpg";
					break;
				case "CreateHTML":
					imgLocation = "http://idmgmtapp01/images/ODDCreateHTMLLayout.jpg";
					break;
				case "ConnectToAzure":
					imgLocation = "http://idmgmtapp01/images/ODDConnectToAzure.jpg";
					break;
				case "MoveFiles":
					imgLocation = "http://idmgmtapp01/images/ODDMoveFiles.jpg";
					break;
				case "FinishUp":
					imgLocation = "http://idmgmtapp01/images/ODDFinishUp.jpg";
					break;
			}
...
function UpdateTerminationDropDown(textBox)
{
	const requesterName = textBox.value;
	const params = 'requesterName=' + requesterName;
	let dropDownData = '';
...
function DisplayFormerAssociateInfo(requesterName)
{
	const params = 'requesterName=' + requesterName;
	const request = new XMLHttpRequest();
	let myTable = '';
	request.addEventListener('readystatechange', () => {
		if(request.readyState === 4 && request.status === 200) {
			...
			for(let i = 0;i < arrayLength;i++)
			{
				...
				var HRLegalName = obj.JSON_FormerAssociateInfo[i].HRLegalName;
	...
	request.open("POST", GetFormerAssociateInfoPHPFile, true);
	request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	request.send(params);
}
