...
// Get record count
$CountNumRecords = "select count(*) as count from Ultipro_ADRpt u left join profile p on (u.AssociateID = p.EMPLID) where u.PositionStatus = 'Terminated' and u.TerminationDate > DATEADD(day, -60, GETDATE()) and p.Email is not NULL;";
$rs = odbc_exec($conn,$CountNumRecords);
odbc_fetch_row($rs);
$NumRecords = odbc_result($rs,$Count);

$sql="select lower(Email) as Email from Ultipro_ADRpt u left join profile p on (u.AssociateID = p.EMPLID) where u.PositionStatus = 'Terminated' and u.TerminationDate > DATEADD(day, -60, GETDATE()) and p.Email is not NULL order by Email;";
