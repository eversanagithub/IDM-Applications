...
my @JD = split(';', $jobDescription);
my $jobTitle = $JD[0];
my $jobFamilyCode = $JD[1];
my $jobLocation = $JD[2];
my $jobID = $JD[3];
...
sub LogTheEvent
{
	...
	$SQLString = "insert into WebIDMWebsiteLoggedEvents(ExecutedBy,application,time_of_execution,description) values('$WhoAmI','AD Account Creation','$CurrentTimeStamp','Added user $firstName $lastName to Active Directory');";
	$dbh = DBI->connect($dsn,$username,$password, \%attr);
	$sth = $dbh->prepare($SQLString);
	$sth->execute();
	$dbh->disconnect;	
...
{
	$SQLString = "exec AD_Universal_IDMUI_CreateAccount '$firstName','$lastName','$jobID','$manager';";
	$dbh = DBI->connect($dsn,$username,$password, \%attr);
	$sth = $dbh->prepare($SQLString);
	$sth->execute();
	sleep 1;
	$SQLString = "exec msdb.dbo.sp_start_job N'IDM - AD_AdHoc Account Provisioning UI - ADD Universal';";
	$dbh = DBI->connect($dsn,$username,$password, \%attr);
	$sth = $dbh->prepare($SQLString);
	$sth->execute();
}
