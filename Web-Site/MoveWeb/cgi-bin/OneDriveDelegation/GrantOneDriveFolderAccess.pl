...
my $DAP = "WebProdDelegatesAlreadyProcessed";
my $ProcessRequest = "WebProdProcessAccessRequest";
my $SAP = "WebProdStatusOfODDProgress";
my $IDMWebsiteLog = "WebProdIDMWebsiteLoggedEvents";

$dbh = DBI->connect('dbi:ODBC:DSN=ProdDBWebConnection;MARS_Connection=Yes;');
...
sub ProcessFileAccessRequest
{
	if($Employee ne '')
	{
		...
		# Insert record into the 'IDM_Website_Logged_Events' SQL table
		$dbh = DBI->connect('dbi:ODBC:DSN=ProdDBWebConnection;MARS_Connection=Yes;');
		$sth = $dbh->prepare("insert into $IDMWebsiteLog(userID,application,source_user,target_user,ODDAccessType,time_of_execution) values ('$userID','$application','$Employee','$PersonRequestingAccess','$Action','$Today');");
		$sth->execute();
		$dbh->disconnect;
