...
$DAP = "WebProdDelegatesAlreadyProcessed"
$SAP = "WebProdStatusOfODDProgress"
$ProcessRequest = "WebProdProcessAccessRequest"
...
if($PendingRecords -gt 0)
{
	...
	if($ProcessODD -eq "Yes")
	{
		...
		Set-SPOUser -site $URL -LoginName $secondaryAdmin -IsSiteCollectionAdmin $True|Out-File -Filepath C:\temp\junk.txt
	...
	Disconnect-PnPOnline|Out-File -Filepath C:\temp\junk.txt
