...
$SQLString = "declare @cycleRun varchar(20), @Name varchar(300), @perc varchar (3); 
    declare c_reviewLoop cursor for
    select cyclerun from accessreview..cyclerun where cycleEnded is null and cyclestatus = '1'
    open c_reviewLoop fetch next from c_reviewLoop into @cycleRun
    while @@FETCH_STATUS = 0
    Begin
    set @Name = (select cycleNotes from AccessReview..CycleRUN where cyclerun = @cycleRun)  
    set @perc = (select convert(varchar ,cast(A.r1 / B.r2 * 100 as int)) from 
                (select cast(count(AcceptReject) as float) as r1 from accessreview..ReviewData_AcceptReject where CycleRun = @cycleRun ) A,
                (select cast(count(RecordID) as float) as r2 from accessreview..ReviewData_AcceptReject where CycleRun = @cycleRun) B)
        select @name, @perc
        Fetch NEXT from c_reviewLoop into @cycleRun
    end
    close c_reviewLoop
    deallocate c_reviewLoop;";
...
while (@row = $sth->fetchrow_array())
{
  ...
  $SecondOutput = $row[1];
  $ThirdOutput = $row[2];
  print "FirstOutput = [$FirstOutput]\n";
  print "SecondOutput = [$SecondOutput]\n";
  print "ThirdOutput = [$ThirdOutput]\n";
}
$dbh->disconnect;
exit;
#####################################################################
...
sub LogTheEvent
{
	...
	$SQLString = "insert into WebIDMWebsiteLoggedEvents(ExecutedBy,application,time_of_execution,description) values('$WhoAmI','AssociateTermination','$CurrentTimeStamp','Processed termination of $POST_FN $POST_LN');";
	$dbh = DBI->connect($dsn,$username,$password, \%attr);
	$sth = $dbh->prepare($SQLString);
	$sth->execute();
	$dbh->disconnect;	
