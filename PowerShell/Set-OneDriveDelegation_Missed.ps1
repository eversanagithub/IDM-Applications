...
$HTMLFile = "C:\UtilityScripts\HTML\HTMLFile_$ThisDate.txt"
...
$con = New-Object System.Data.Odbc.OdbcConnection $connStr
$con.Open()
$sql = "select UserName, UPN, ManagerUPN from WebTempTerminatedListing"
...
while ($rdr.Read())
{
	...
	if($delegate -ne '' -and $delegate -ne $null)
	{
		...
		<#
			Write-Host "General Error occurred. Error is:"
			Write-Host
			Write-Host $_
