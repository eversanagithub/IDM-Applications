...
$Option = SQLRead -SQLCommand "select Action from $SQLTable where Selected"
$ThresholdDays = SQLRead -SQLCommand "select ThresholdDays from $SQLTable where Selected"
$WorkSheetBaseName = SQLRead -SQLCommand "select WorkSheetBaseName from $SQLTable where Selected"
$ExcelSpreadsheetFileName = SQLRead -SQLCommand "select ExcelSpreadsheetFileName from $SQLTable where Selected"
$ExcelSpreadsheetFileLocation = SQLRead -SQLCommand "select ExcelSpreadsheetFileLocation from $SQLTable where Selected"
...
$FilePath = $ExcelSpreadsheetFileLocation + $ExcelSpreadsheetFileName
...
switch($Option)
{
	...
	VerifyDisabledStaleDevices
	{
		#Write-Host "Processing VerifyDisabledStaleDevices Module"
		$DeviceQuery = Get-MsolDevice -all -ReturnRegisteredOwners -LogonTimeBefore $lastLogon | where{$_.Enabled -eq $false} | select Enabled, ObjectId, DeviceId, DisplayName, DeviceOsType, DeviceOsVersion, DeviceTrustType, DeviceTrustLevel, ApproximateLastLogonTimestamp, DirSyncEnabled, LastDirSyncTime, @{Name='Registeredowners';Expression={[string]::join(";", ($_.Registeredowners))}}
		$DeviceQuery | Export-Excel -workSheetName $WorkSheetBaseName -path $FilePath -ClearSheet -FreezeTopRow -TableName "AADDevicesTable" -TableStyle Medium16 -AutoSize
		break
	}
	DisableStaleDevices
	{
		Write-Host "Processing DisableStaleDevices Module"
		$DeviceQuery = Get-MsolDevice -all -ReturnRegisteredOwners -LogonTimeBefore $lastLogon | where{$_.Enabled -eq $true} | select ObjectId, DeviceId, DisplayName, DeviceOsType, DeviceOsVersion, DeviceTrustType, DeviceTrustLevel, ApproximateLastLogonTimestamp, DirSyncEnabled, LastDirSyncTime, @{Name='Registeredowners';Expression={[string]::join(";", ($_.Registeredowners))}} 
		$DeviceQuery | Disable-MsolDevice -Force
		$DeviceQuery | Export-Excel -workSheetName $WorkSheetBaseName -path $FilePath -ClearSheet -FreezeTopRow -TableName "AADDevicesTable" -TableStyle Medium16 -AutoSize
		break
	}
	RemoveStaleDevices
	{
		Write-Host "Processing RemoveStaleDevices Module"
		$DeviceQuery = Get-MsolDevice -all -ReturnRegisteredOwners -LogonTimeBefore $lastLogon | where{$_.Enabled -eq $true} | select ObjectId, DeviceId, DisplayName, DeviceOsType, DeviceOsVersion, DeviceTrustType, DeviceTrustLevel, ApproximateLastLogonTimestamp, DirSyncEnabled, LastDirSyncTime, @{Name='Registeredowners';Expression={[string]::join(";", ($_.Registeredowners))}} 
		$DeviceQuery | Disable-MsolDevice -Force
		$DeviceQuery = Get-MsolDevice -all -ReturnRegisteredOwners -LogonTimeBefore $lastLogon | select ObjectId, DeviceId, DisplayName, DeviceOsType, DeviceOsVersion, DeviceTrustType, DeviceTrustLevel, ApproximateLastLogonTimestamp, DirSyncEnabled, LastDirSyncTime, @{Name='Registeredowners';Expression={[string]::join(";", ($_.Registeredowners))}} 
		$DeviceQuery | Remove-MsolDevice -Force
		$DeviceQuery | Export-Excel -workSheetName $WorkSheetBaseName -path $FilePath -ClearSheet -FreezeTopRow -TableName "AADDevicesTable" -TableStyle Medium16 -AutoSize
		break
	}
}
