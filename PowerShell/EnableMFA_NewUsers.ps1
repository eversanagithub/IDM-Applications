...
###############################################################
#		Step 5: Create SQL table if it does not currently exist		#
###############################################################
...
###############################################################
#		Step 6: Find the GUID for each f the four groups.					#
###############################################################
...
###############################################################
#		Step 7: Process each user in the Office365_NoMFA Group.		#
###############################################################
...
###############################################################
#		Step 8: Disconnect to Azure Resources.										#
###############################################################
...
###############################################################
#		Step 9: Find user already in MFA which need SSPR changes.	#
###############################################################
# Create the credentials for AzureAD
$serviceAccountUserName3 = Get-Content "C:\powershell\credentials\SRV_SSPR_UserName.txt"
$EncryptedPasswordFile3 = SQLReadEncryption -SQLCommand "select filepath from $EncryptionSQLTable where currentUser = '$currentUser' and serviceAcct = '$serviceAccountUserName2'"
...
$credential3 = New-Object System.Management.Automation.PSCredential($serviceAccountUserName3,$serviceAccountPassword3)

# Connect to Azure Active Directory
Connect-AzureAD -Credential $credential3|Out-File -Filepath C:\temp\junk.txt
