...
#################################################
#		Step 8: Initiate Storage Table Connections	#
...
##################################################
#    Step 9: Pull previously processed delegates #
##################################################
...
#################################################
#    Step 10: Get all personal OneDrive sites   #
#################################################
...
#################################################
#    Step 11: Get all current disabled users    #
#################################################
...
<#
Ted's code which replaces line 545
...
$Disablesquery = "  select COALESCE(asr.FieldValue,sr.FieldValue)+'@eversana.com' as UPN
 ...
 where r.targetid = 'ad_universal'  and r.Status = 'Completed' and r.Action = 'DIS' and r.ProcessedDate is not null and r.requestdate > convert(varchar, getdate() - 1, 112)
UNION
 select aad.UPN
 From Request_VW r 
 inner join profile p on (p.EversanaID = r.EversanaID)
 inner join feed_ad_azure aad on (aad.employeeNumber = p.emplid)
 where r.targetid = 'ad_universal'  and r.Status = 'Completed' and r.Action = 'DIS' and r.ProcessedDate is not null and r.requestdate > convert(varchar, getdate() - 1, 112)" 
...
#################################################
#    Step 12: Create list of eligable delegates #
#################################################
...
#################################################
#    Step 13: Process eligable delegates        #
#################################################
...
foreach ($delegate in $delegates)
{
	Try
	{
		...
		# Windows File Explorer, the only thing you see is a folder called 'TERM-2023-03-01-JOHN_SMITH'. Everything that used to be in
...
#################################################
#    Step 14: Send out Manager reminder E-Mails #
#################################################
...
#################################################
#    Step 15: Disconnect from Azure services    #
#################################################
